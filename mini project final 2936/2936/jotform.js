...
//                        p.$jot('.jt-content, .jt-title').css('width', 'auto');
   ...
   */
    ...
    jumpToPage: function () {
        ...
        if(window.location.href.indexOf("doubleValidation") > -1) {
    ...
    isSectionTouched: function (section) { 
        ...
        function filterFields(el) {
            ...
            var isDefaultValue = JotForm.defaultValues[el.id] === el.value;

            return !alwaysHidden && !isDefaultValue;
        }

        var inputs = section.querySelectorAll('*:not(button)[id^="input_"], input[id^="first_"], input[id^="last_"], input[id^="lite_mode_"]').filter(filterFields);
        var filled = 0;
        ...
        for (var i = 0; i < inputs.length; i++) {
            ...
            if (fieldType === 'control_rating') {
                var starField = input.querySelector('.form-star-rating-star');
                ...
                if (input.tagName == 'INPUT' && input.readAttribute('type') == "file") { // Upload
                    ...
                    try {
                        ...
                        var checkValues = inputs.map(function (e) {
                            ...
                            if (window.location.href.indexOf("doubleValidation") > -1) {
                                var _isVisible = !(JotForm.getContainer(e).hasClassName('always-hidden') || JotForm.getContainer(e).hasClassName('form-field-hidden'));
                            }
